...
public class ResilienceHandler {
	...
	private CircuitBreaker configureCircuitBreaker(String serviceName) {
	...
	private CircuitBreaker configureCircuitBreaker() {
		return configureCircuitBreaker("taxservice");
	}
	...
	private Retry configureRetry() {
		return configureRetry("taxservice");
	}

	private Retry configureRetry(String servicename) {
		RetryConfig retryConfig =
				RetryConfig.custom()
						   .maxAttempts(3)
						   .waitDuration(Duration.ofMillis(5000)).build();
		Retry retry = Retry.of(servicename, retryConfig);
		return retry;
	}
	...
	public String applyResiliencePatternsForLogisticsService(String shippedFrom, String shippedTo, String note) {

		logger.info("Applying resilience patterns for logistics service");
		CircuitBreaker circuitBreaker = configureCircuitBreaker("logisticsservice");
		TimeLimiter timeLimiter = configureTimeLimiter();
		Retry retry = configureRetry("logisticsservice");

		Supplier<CompletableFuture<String>> futureSupplier = () -> CompletableFuture.supplyAsync(() -> salesOrderService.getFinalShipmentMode(shippedFrom, shippedTo, note));
		Callable<String> callable = TimeLimiter.decorateFutureSupplier(timeLimiter, futureSupplier);
		callable = CircuitBreaker.decorateCallable(circuitBreaker, callable);
		callable = Retry.decorateCallable(retry, callable);

		//Executing the decorated callable and recovering from any exception by calling the fallback method
		Try<String> result = Try.ofCallable(callable).recover(throwable -> logisticsServiceFallback(note));
		return result.get();
	}

	protected String logisticsServiceFallback(String note) {
		logger.info("Logistics service is down. So a default mode of transport will be AIR");
		return note + " " +"AIR";
	}
}
